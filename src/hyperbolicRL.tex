...
As seen in \Cref{sec:gnn} regarding GNNs, the neighbourhood aggregation step (\Cref{eq:neighAgg}) is crucial to capture neighbourhood structures and features. The analogue of mean aggregation in Euclidean GNNs (\Cref{eq:aggregateBase}) in hyperbolic space is the Fréchet mean~\cite{frechet1948elements}, which alas has no closed form solution. Therefore, the authors of HGCN perform aggregation in tangent spaces using hyperbolic attention.
...
Then the nodes' representations are averaged, like in the basic GNN. The aggregation step can be performed in two flavours.
\begin{description}
    \item[global] 
