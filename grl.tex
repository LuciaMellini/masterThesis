...
\subsection{Generalized Neighbourhood Aggregation}
...
\subsubsection{AGGREGATE generalization}
...
\paragraph{Set Aggregators}
In GNNs neighborhood aggregation must be permutation-invariant, as node neighborhoods have no inherent order. Traditional methods, such as simple summation, may limit the expressive power of GNNs. 

\term{Set pooling}~\cite{zaheer2017deepSets}, applies a multi-layer perceptron (MLP) to each neighbor’s embedding before summing the results and passing them through another MLP. This method ensures universal function approximation while maintaining permutation invariance. 

\term{Janossy pooling}~\cite{murphy2018janossyPooling}, takes a different approach. Instead of relying on permutation-invariant operations like summation, it applies a permutation-sensitive function, such as an LSTM~\cite{hochreiter1997LSTM}, to ordered sequences of neighbor embeddings. Since computing all possible permutations is infeasible, practical implementations either sample a subset of permutations or use a fixed canonical ordering, such as sorting nodes by degree.
