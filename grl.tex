...
Given a graph $G=(V,E,L)$ in \term{node classification} the goal is to predict the label associated with each node $v\in V$, when we only given the true labels on a \term{training set} of nodes $V_\text{train}\subset V$. Often one assumes that we only have label information for a small subset of nodes, but there are instances that involve many labeled nodes, like classifying the function of proteins in the interactomes of different species~\cite{Hamilton2017inductiveRepresentationLearning}.
...
\section{Embeddings for KGs}\label{sec:embeddingsKGs}
...
The concepts and architectures described in the following sections can be extended for multi-relational graphs, similarly those presented in \Cref{sec:embeddingsKGs}.
...
\end{equation*}
where $\mathbf{h}_u$ represents the node’s prior representation, and $\mathbf{m}_\mathcal{N}(u)$ denotes the aggregated message from neighbors and $\text{UPDATE}_{\text{base}}$ refers to the update function defined in \Cref{eq:updateBase}. This method, introduced in the \textit{GraphSAGE} framework~\cite{Hamilton2017inductiveRepresentationLearning}, aims to disentangle neighborhood and node-specific information.

An alternative skip-connection method utilizes linear interpolation~\cite{Pham2017columnNetworksCollectiveClassification}:
\begin{equation*}
...
\end{equation*}
where $\circ$ stands for componentwise multiplication and $ \alpha_1 \in [0,1]^d $ is a gating vector that may be learned via an auxiliary GNN~\cite{Pham2017columnNetworksCollectiveClassification}, a multi-layer perceptron (MLP), or direct optimization.

These techniques mitigate over-smoothing and enhance numerical stability in GNN training. They are particularly beneficial for node classification tasks in moderately deep GNN architectures (2-5 layers) and perform well in homophilic settings, where node labels correlate with local neighborhood features.

\subsubsection{Gated Updates}
One way of viewing the GNN message passing algorithm is that the aggregation function is receiving an \term{observation} from the neighbours, which is then used to update the \term{hidden state} of each node. In this view inspired by \term{recurrent neural networks} (\term{RNN}), one can apply methods used to update the hidden state of RNN architectures based on observations. For instance, one of the earliest of these GNN architectures defines the update function as

\begin{equation*}
    \mathbf{h}_u^{(k)} = \text{GRU}(\mathbf{h}_u^{(k-1)}, \mathbf{m}_{\mathcal{N}(u)}^{(k)})
\end{equation*}

where GRU denotes the update equation of the \term{gated recurrent unit} cell~\cite{Cho2014learningPhraseRepresentations}. Alternatively one could use updates base on LSTM architecture~\cite{selsam2018LearningSATSolverSingleBitSupervision}, or any update function defined for RNNs.

\subsubsection{Jumping Knowledge Connections}
Until now we have assumed that the node representations used in a downstream task are those outputted by hte final layer of the GNN. To improve the quality of the final node representations one can leverage the representations at each layer of message passing, so that the final node representations are as follows:
\begin{equation*}
    \mathbf{z}_u = f_\text{JK}(\mathbf{h}_u^{(0)} \oplus \mathbf{h}_u^{(1)} \oplus ... \oplus \mathbf{h}_u^{(K)}),
\end{equation*}
where $f_\text{JK}$ is an arbitrary differentiable function. This strategy is known as \term{jumping knowledge} (\term{JK}) \term{connections}~\cite{xu2018RepresentationLearningGraphsJumpingKnowledgeNetworks}.


\subsection{Generalized Message Passing}
Until now we have focused on GNN message passing that operates at the node level. The message passing approach can be generalized to leverage edge and graph-level information. In the more general approach~\cite{battaglia2018relationalInductiveBiases} the following equations define each iteration of message passing:
\begin{align*}
...
The key innovation in this generalized message passing framework is that during message passing we generate hidden embeddings $\mathbf{h}^{(k)}_{(u,v)}$ for each edge in the graph, as well as an embedding $\mathbf{h}^{(k)}_G$ corresponding to the entire graph. The quantities are updated in the order in which the respective equations are listed above. All of the individual update and aggregation operations in such a generalized message-passing framework can be implemented using the techniques discussed in this section.